...
func CreateCsrfToken(ctx *gin.Context) (csrfToken string) {
	...
	csrfToken = util.GetSessionStr(session, "csrfToken")
	if csrfToken == "" {
		nowTime := time.Now().String()          //获取当前时间
		csrfToken, _ = util.EncryptPwd(nowTime) //根据当前时间生成 csrfToken
...
func verifyCsrfToken(ctx *gin.Context) bool {
	// 请求方法无需校验 csrfToken
	if util.IsContain(allowedMethods, ctx.Request.Method) {
		return true
	}
	...
	correctCsrfToken := util.GetSessionStr(sessions.Default(ctx), "csrfToken")
	// 校验 csrfToken
