...
export default {
  ...
  actions: {
     ...
     * @param {Object} payload pwd {String} 密码
     * @param {Object} payload captchaId {String} 验证码 ID
     * @param {Object} payload captchaVal {String} 验证码
    ...
    login ({ dispatch }, {
      ...
      return new Promise((resolve, reject) => {
        authLogin({
          ...
          .then(async res => {
            const data = res.data
            // 设置 localStorage 一定要存 uuid 和 token 两个 cookie
            // 整个系统依赖这两个数据进行校验和存储
